...
{
    public class MucTinController : OfficeController
    {
        [Inject]
        public IMucTinRepository MucTinRepository { get; set; }
        ...
        {
            var items = MucTinRepository.GetAll().Select(x => x.ToDataViewModel()).ToList();
        ...
        {
            var data = MucTinRepository.Single(id).ToDataViewModel();
